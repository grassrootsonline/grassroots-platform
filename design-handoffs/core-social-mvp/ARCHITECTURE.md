...
| **Styling** | Tailwind CSS 4 + CSS custom properties from the Grassroots design system |
...
| **Button press** | `whileTap: { scale: 0.97 }` on interactive buttons. |
| **Notification bell** | Bounce animation (keyframes) when a new notification arrives. |
| **Like / react button** | Spring animation on `scale: 1.25` then back to `1` on reaction. |
| **Infinite scroll** | New items enter with `opacity 0` to `1`, no position shift (prevents layout jump). |

### 12.3 Design token rules

All colors, spacing, typography, and radius values must reference CSS custom properties defined in `styles/tokens.css` and the Grassroots design system. Hard-coded hex values in component files are forbidden. Tailwind classes must map to these tokens via the Tailwind config.
...
- No inline styles. Tailwind classes mapped to design tokens only.
